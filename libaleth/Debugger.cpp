...
bool DebugSession::populate(dev::eth::Executive& _executive, dev::eth::Transaction const& _transaction)
{
	...
	h256 lastHash;
	h256 lastDataHash;
	auto onOp = [&](uint64_t steps, uint64_t PC, Instruction inst, bigint newMemSize, bigint gasCost, bigint gas, VM* voidVM, ExtVMFace const* voidExt)
	{
		VM& vm = *voidVM;
		...
		history.append(WorldState({steps, ext.myAddress, PC, inst, newMemSize, static_cast<u256>(gas), lastHash, lastDataHash, vm.stack(), vm.memory(), gasCost, ext.state().storage(ext.myAddress), levels}));
	};
	_executive.go(onOp);
	_executive.finalize();
	return true;
