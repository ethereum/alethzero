...
void NetworkSettings::read(QSettings const& _settings)
{
	...
	privateChainId = _settings.value("privateChain", "").toString();
	dropPeers = _settings.value("dropPeers", false).toBool();
	privateChain = _settings.value("usePrivateChain", !privateChainId.isEmpty()).toBool();
	p2pSettings.discovery = (!privateChain || privateChainId.isEmpty()) && !hermitMode;
...
void NetworkSettings::write(QSettings& _settings) const
{
	...
	_settings.setValue("privateChain", privateChainId);
	_settings.setValue("usePrivateChain", privateChain);
}
...
NetworkSettings NetworkSettingsPage::prefs() const
{
	...
	ret.privateChain = m_ui->useNetworkId->isChecked();
	ret.privateChainId = m_ui->networkId->text();
	return ret;
}
...
void NetworkSettingsPage::setPrefs(NetworkSettings const& _settings)
{
	...
	m_ui->useNetworkId->setChecked(_settings.privateChain);
	m_ui->networkId->setText(_settings.privateChainId);
}
